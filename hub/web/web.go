...
var htmlTemplates = template.Must(template.New("htmlTemplates").ParseGlob("hub/templates/html/*.html"))
var jsModelTemplates = template.Must(template.New("jsModelTemplates").ParseGlob("hub/templates/js/models/*.js"))
var jsCollectionTemplates = template.Must(template.New("jsCollectionTemplates").ParseGlob("hub/templates/js/collections/*.js"))
var jsViewTemplates = template.Must(template.New("jsViewTemplates").ParseGlob("hub/templates/js/views/*.js"))
var _Templates = template.Must(template.New("_Templates").ParseGlob("hub/templates/_/*.html"))
var jsTemplates = template.Must(template.New("jsTemplates").ParseGlob("hub/templates/js/*.js"))
var cssTemplates = template.Must(template.New("cssTemplates").ParseGlob("hub/templates/css/*.css"))
...
func handleStatic(router *mux.Router, dir string) {
	...
	for _, fil := range children {
		...
		router.MatcherFunc(func(r *http.Request, rm *mux.RouteMatch) bool {
			...
			if in, err := os.Open(filepath.Join("hub/static", cpy)); err != nil {
				w.WriteHeader(500)
			} else {
				defer in.Close()
...
func main() {
	...
	handleStatic(router, "hub/static")
